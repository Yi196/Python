...
def _detect_roi_mid(gray, orient, brightness, max_pts_num, min_pts_num, thresh, is_subpixel=True, n=5):
    '''
        one image, one roi
        返回值：
    ...
    if orient in ['从右到左', '从左到右']:
        max_h = np.argmax(gray, axis=1)
        pts = np.stack([max_h, np.array(range(gray.shape[0]), np.int32)], axis=0)  # pts=[[x1,x2..],[y1,y2..]]
        num = max_h.shape[0]
        if num > max_pts_num:
        ...
        if is_subpixel:
            ...
            for i in range(used_num):
                cur_y = pts[1, used[i]]
                cur_x = pts[0, used[i]]
                if gray[cur_y, cur_x] < thresh:
                    continue
                if cur_x == 0 or cur_x == w - 1:
                    sub_pts.append((cur_x, cur_y))
                else:
                    val = gray[cur_y, cur_x - 1] + gray[cur_y, cur_x + 1] - 2 * gray[cur_y, cur_x]
                    sign = np.sign(val)
                    if sign == 0:
                        sign = 1
                    sub_offset = (gray[cur_y, cur_x - 1] - gray[cur_y, cur_x + 1]) / (sign * max(0.0001, abs(val))) / 2
        ...
        if pts.shape[0] - n < min_pts_num or pts.shape[1] != 2:
            ...
            line_point = _coeff2pts(gray, coeff[1:])
            # 删除n个异常点
        ...
        max_w = np.argmax(gray, axis=0)
        pts = np.stack([np.array(range(gray.shape[1]), np.int32), max_w], axis=0)
        num = max_w.shape[0]
        if num > max_pts_num:
            used = np.linspace(0, num, max_pts_num, endpoint=False)
            used = used.astype(np.int32)
        ...
        if is_subpixel:
            ...
            for i in range(used_num):
                ...
                if cur_y == 0 or cur_y == h - 1:
                    ...
                    val = gray[cur_y - 1, cur_x] + gray[cur_y + 1, cur_x] - 2 * gray[cur_y, cur_x]
                    sign = np.sign(val)
                    if sign == 0:
                        sign = 1
                    sub_offset = (gray[cur_y - 1, cur_x] - gray[cur_y + 1, cur_x]) / (sign * max(0.0001, abs(val))) / 2
...
def detect_line_subpixel(img,
    ...
    brightness: str '从白到黑','从黑到白','暗中线','亮中线'
    ...
    if '中线' in brightness:
        line_point, coeff, pts = _detect_roi_mid(roi, orient, brightness, max_pts_num + n, min_pts_num, mag_thresh, is_subpixel, n)
    else:
        line_point, coeff, pts = _detect_roi(roi, orient, brightness, max_pts_num + n, min_pts_num, mag_thresh, is_subpixel, n)
...
if __name__ == '__main__':
    ...
    mag_thresh = 50
    n = 5
    line_point, coeff, pts, angle = detect_line_subpixel(img, rect, orient, brightness, max_pts_num, min_pts_num, mag_thresh, n=n)
    print(line_point,'\n',coeff,'\n',pts)
    print('直线倾斜角度为',angle)
    x0, y0, x1, y1 = line_point

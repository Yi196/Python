...
def _detect_roi(image, orient, max_pts_num, min_pts_num, mag_thresh, is_subpixel=True, n=5):
    '''
    one image, one roi
    返回值：
...
def detect_line_subpixel(binary,
                         rect,
                         orient='从左到右',
                         brightness='从白到黑',
                         max_pts_num=80,
    ...
    该算法用于检测orient方向上的第一条直线，用于线段后方有空缺且梯度可能大于直线处梯度时

    images: 输入图像
    rect: list [x0, y0, x1, y1]  找线区域
    ...
    line_point, coeff, pts = _detect_roi(roi, orient, max_pts_num, min_pts_num, mag_thresh, is_subpixel,n)
    ...
    if coeff[0] == 0:
        ...
        # # 显示用来拟合直线的点
        # img = np.stack([binary, binary, binary], axis=-1).astype(np.uint8)
        # for x, y in pts:
...
if __name__ == '__main__':
    # img = cv2.imread(r'../001.jpg')
    path_img = r'C:\Users\dihuge\Desktop\989_roi_2_col0'
    names = os.listdir(path_img)
    for i in names:
        img = cv2.imread(os.path.join(path_img, i))
        img_gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        _,binary = cv2.threshold(img_gray,60,255,cv2.THRESH_BINARY)
        # cv2.imshow('',binary)
        # cv2.waitKey(0)
        rect = [1852, 790, 1998, 850]
        orient = '从下到上'
        brightness = '从白到黑'
        max_pts_num = 80
        min_pts_num = 5
        mag_thresh = 50
        line_point, coeff, pts, angle = detect_line_subpixel(binary, rect, orient, brightness, max_pts_num, min_pts_num, mag_thresh)
        # print(line_point,'\n',coeff,'\n',pts)
        print('直线倾斜角度为',angle)
        x0, y0, x1, y1 = line_point
        cv2.rectangle(img, (rect[0], rect[1]), (rect[2], rect[3]), (0, 255, 0), 2)
        x0, y0, x1, y1 = line_point
        cv2.line(img, (int(x0), int(y0)), (int(x1), int(y1)), (0, 0, 255), 4)
        # for x,y in pts:
        #     cv2.circle(img,(int(x),int(y)),2,(0,0,255),-1)
        cv2.namedWindow('123',0)
        cv2.resizeWindow('123',630,400)
        cv2.imshow('123',img)
        cv2.waitKey(0)
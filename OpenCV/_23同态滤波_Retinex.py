...
def my_homofilter(img, gammaL=0.5, gammaH=5, C=0.1):
    if len(img.shape) > 2:
        if img.shape[2] == 3:
    ...
    # 1、取对数
    img_log = np.log(img + 1e-5)
    # 2、傅里叶变换
    fft_shift = np.fft.fftshift(np.fft.fft2(img_log))
    # 3、频域滤波
    ...
    #     for j in range(cols):
    #         d2 = (i - n1) ** 2 + (j - n2) ** 2
    #         H[i, j] = (gammaH - gammaL) * (1 - np.exp(-C * d2 / (2 * D0 ** 2))) + gammaL  # %高斯同态滤波
    # print('H', H)
    ...
    # 傅里叶逆变换
    ifft = np.fft.ifft2(np.fft.ifftshift((H * fft_shift)))
    # 指数运算
    img_exp = np.exp(ifft) - 1
    # 将复数转为0-255整数
...
def my_homofilter_RGB(img, gammaL=0.5, gammaH=5, C=0.1):
    B, G, R = cv2.split(img)
    img_ret_B = my_homofilter(B, gammaL, gammaH, C)
    img_ret_G = my_homofilter(G, gammaL, gammaH, C)
    img_ret_R = my_homofilter(R, gammaL, gammaH, C)

...
# cap = cv.VideoCapture('./image/002.avi')
cap = cv.VideoCapture(0)

#通过 print(cv.__file__) 查看文件位置
# print(cv.__file__)

#定义分类器（人脸识别）
classifier_face = cv.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_frontalface_alt.xml")
#定义分类器（人眼识别）
classifier_eye = cv.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_lefteye_2splits.xml")
#定义分类器（嘴巴识别）
# classifier_mouth=cv.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_mcs_mouth.xml")
...
while cap.isOpened():
    ...
    if ret==True:
        ...
        # 检测器：detectMultiScale参数（图像，每次缩小图像的比例，匹配成功所需要的周围矩形框的数目，检测的类型，匹配物体的最小尺寸）
        faceRects_face = classifier_face.detectMultiScale(img_gray, 1.2, 2, cv.CASCADE_SCALE_IMAGE, (20, 20))
        ...
        if len(faceRects_face) > 0:
            for faceRect_face in faceRects_face:
                ...
                face_gary =img_gray[y:y+h,x:x+w]

                # faceRects_mouth = classifier_mouth.detectMultiScale(face_gary, 1.1, 1, cv.CASCADE_SCALE_IMAGE, (5, 20))
                # #嘴巴检测器
                # if len(faceRects_mouth) > 0:
                #     for faceRect_mouth in faceRects_mouth:
                #         xm1, ym1, wm1, hm2 = faceRect_mouth
                #         cv.rectangle(face_img, (xm1, ym1), (xm1 + wm1,ym1 + hm2), (0, 255,0), 2)


                faceRects_eye = classifier_eye.detectMultiScale(face_gary, 1.2, 2, cv.CASCADE_SCALE_IMAGE, (10, 10))
                #检测器识别眼睛
                if len(faceRects_eye) > 0:
                    ...
                    for faceRect_eye in faceRects_eye:
                        ...
                        # 画出眼睛区域
                        cv.rectangle(face_img, (x1, y1), (x1 + w1,y1 + h2), (0, 255,0), 2)
...
#获取外接摄像头
eye = cv2.imread(r"./OpenCV/image/007.jpg")
#读取眼睛区域替换的图片
mouth = cv2.imread(r"./OpenCV/image/005.jpg")
#读取嘴巴区域替换的图片
...
classifier_face = cv2.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_frontalface_alt.xml")
#定义分类器（人脸识别）
classifier_eye = cv2.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_eye.xml")
#定义分类器（人眼识别）
classifier_mouth=cv2.CascadeClassifier("C:/ProgramData/Anaconda3/envs/chango/lib/site-packages/cv2/data/haarcascade_mcs_mouth.xml")
#定义分类器（嘴巴识别）
